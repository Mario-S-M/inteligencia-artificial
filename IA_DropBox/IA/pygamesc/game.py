...
def disparar_bala():
    ...
    if not bala_disparada:
        # Velocidad aleatoria pero siempre positiva (aunque internamente necesitamos que sea negativa)
        velocidad_bala = random.randint(3, 8) * -1  # Velocidad aleatoria negativa para la bala
        bala_disparada = True
...
def guardar_datos():
    ...
    velocidad_absoluta = abs(velocidad_bala)  # Valor absoluto para tener velocidad positiva
    salto_hecho = 1 if salto else 0  # 1 si saltó, 0 si no saltó
    
    # Guardar velocidad de la bala (positiva), distancia al jugador y si saltó o no
    datos_modelo.append((velocidad_absoluta, distancia, salto_hecho))

# Función para guardar los datos en un archivo CSV
def guardar_csv():
    # Crear un nombre de archivo con la fecha y hora actuales
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ruta_archivo = f"datos_juego_{timestamp}.csv"
    ...
    try:
        with open(ruta_archivo, mode='w', newline='') as archivo:
            ...
            # Escribir todos los datos recopilados
    ...
    while menu_activo:
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                if datos_modelo:  # Si hay datos, guárdalos antes de salir
                    guardar_csv()
                pygame.quit()
                exit()
...
def reiniciar_juego():
    ...
    # Guardar los datos en CSV al perder
    guardar_csv()
    ...
    # Limpiar datos para la siguiente partida
    datos_modelo.clear()
